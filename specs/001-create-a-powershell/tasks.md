...
Constitution path rule: All scripts MUST avoid absolute filesystem paths. Resolve paths at runtime using `$PSScriptRoot` or a repository-root helper. Tasks below use repository-root–relative paths (e.g., `tools/...`, `tests/...`) and expect scripts to compute file locations relative to their own script root.

T001 [X] Setup: Project skeleton and linting (completed)
- Title: Create script skeleton, repo metadata, and quickstart
- Files to create/update:
  - `tools/Install-SpecKitTemplate.ps1` (script entrypoint - ensure comment-based help present)
...
- Files to create:
  - `tests/Install-SpecKitTemplate.Args.Tests.ps1`
...
- Files to create:
  - `tests/Install-SpecKitTemplate.AssetSelection.Tests.ps1`
...
T004 [X] Core: Implement release discovery & asset selection (Get-LatestReleaseTag, Find-ReleaseAsset) (completed)
...
T005 [X] Core: Implement download + retry helper (Download-Asset, Invoke-WithRetry) (completed)
...
T006 [X] Core: Implement ZIP validation and safe extraction (Validate-Zip, Safe-Extract) (completed)
...
- Dependency: T005
-- Success criteria: `tests/Install-SpecKitTemplate.Tests.ps1` passes and extraction handles existing files correctly.

T007 [X] Core: Agent auto-selection and interactive mode (completed)
...
T008 [X] Tests (integration) - Download & extract (dry-run)
...
T009 [X] Logging, exit codes, and help polish
...
Notes:
- Integration test (T008) implemented at `tests/integration/01-download-integration.Tests.ps1`. It creates a sample ZIP and mocks download functions so CI can run the flow without network.
- Exit codes and help (T009) implemented in `tools/spec-kit-downloader.ps1` and documented in `specs/001-create-a-powershell/quickstart.md`.
- CI integration-tests job is gated behind PR label `run-integration` in `.github/workflows/powershell-ci.yml` to conserve CI minutes.

T010 [X] CI: Add/verify GitHub Actions workflow for linting and tests (completed)
...
T012 [X] Manual verification & smoke tests (completed)
...
pwsh -NoProfile -c "Import-Module Pester -RequiredVersion 5.0.0 -Force; $r = Pester\Invoke-Pester -Path tests -PassThru; if ($r.FailedCount -gt 0) { exit 1 }"
